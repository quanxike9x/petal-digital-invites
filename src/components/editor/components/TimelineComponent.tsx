...
/**
 * TIMELINE COMPONENT -> CONVERTED TO MONTHLY CALENDAR GRID (LỊCH THÁNG)
 * Allows selecting Month, Year, Day 1 (Wedding Day), Day 2 (Engagement Day), and highlighting 2 days simultaneously!
 */
export const TimelineComponent: React.FC<ComponentProps> = React.memo(({ props, style }) => {
  const now = new Date();
  ...
  const showTwoDates = props?.showTwoDates === true || props?.showTwoDates === 'true';
  const styleType = (props?.styleType as string) || 'style1';
  const accentColor = (props?.accentColor as string) || (props?.primaryColor as string) || '#e11d48';
  ...
  return (
    <div style={fontStyle} className="w-full text-center font-sans select-none overflow-hidden">
      {/* Month & Year Title */}
      <div className="mb-3">
        <h4 
      ...
      {/* Monthly Days Grid with Highlighted Selected Days */}
            ...
            <div
              ...
              className={`h-8 flex flex-col items-center justify-center transition-all relative ${
                ...
                isWedding || isEngagement
                  ? 'font-bold shadow-md scale-110 ring-2 ring-amber-300'
