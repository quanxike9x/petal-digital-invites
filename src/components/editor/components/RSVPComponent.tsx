...
export const RSVPComponent: React.FC<RSVPComponentProps> = ({ component, props: inputProps, style: inputStyle }) => {
  ...
  const containerStyle: React.CSSProperties = {
    ...
    padding: typeof style.padding === 'number' ? `${style.padding}px` : (style.padding as string) || '24px',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Submit RSVP response to Database (Supabase)
      ...
      // 2. Submit Wish to Database if wish is provided
      if (wish.trim()) {
  ...
  return (
    <div style={containerStyle} className="w-full text-center font-sans select-none overflow-hidden">
      <div className="mb-4">
