...
export const AdminTemplateEditor: React.FC = () => {
  ...
  // 📍 8. ADD COMPONENT AT CLICK POSITION
  const handleAddComponentToSection = (type: WidgetType) => {
    console.log('🟡 BẮT ĐẦU THÊM COMPONENT:', { type, clickPosition });
    ...
    console.log('🟡 COMPONENT TRƯỚC KHI SET POSITION:', newComp.layout?.position);

    // 📍 Nếu có clickPosition, set vị trí cho component mới
    if (clickPosition) {
      const posX = clickPosition.x;
      const posY = clickPosition.y;
      
      console.log('🟡 SỬ DỤNG clickPosition:', { posX, posY });
      
      if (newComp.layout && newComp.layout.position) {
        newComp.layout.position.x = Math.max(0, Math.round(posX));
        newComp.layout.position.y = Math.max(0, Math.round(posY));
        ...
        switch (type) {
          ...
          case 'Video':
          case 'Map':
            newComp.layout.position.width = 320;
            newComp.layout.position.height = 240;
            break;
          case 'Text':
            newComp.layout.position.width = 200;
            newComp.layout.position.height = 'auto';
            break;
          case 'Countdown':
          case 'Timeline':
          case 'RSVP':
            newComp.layout.position.width = 300;
            newComp.layout.position.height = 200;
            break;
          default:
            newComp.layout.position.width = 200;
            newComp.layout.position.height = 120;
        }
        
        console.log('🟢 POSITION ĐÃ SET:', newComp.layout.position);
      }
      
      // Reset clickPosition sau khi dùng
      setClickPosition(null);
      toast.success(`Đã thêm ${type} tại vị trí (${Math.round(posX)}, ${Math.round(posY)})`);
    } else {
      console.log('🔴 KHÔNG CÓ clickPosition, thêm vào vị trí mặc định');
      toast.info(`Đã thêm ${type} vào Section (click canvas để chọn vị trí)`);
    }
    ...
    console.log('🟢 LAYOUT MỚI - Component position:', 
      newSections
        .flatMap(s => getSectionComponents(s))
        .find(c => c.id === newComp.id)?.layout?.position
    );
  ...
  const handleCanvasClick = (x: number, y: number, containerRef: HTMLDivElement | null) => {
    console.log('🔵 ADMIN TEMPLATE EDITOR - CANVAS CLICK:', { x, y, containerRef });
    
    if (containerRef) {
      canvasContainerRef.current = containerRef;
    }
    
    // Lưu vị trí click vào state
    setClickPosition({ x, y });
    console.log('🟢 ĐÃ SET clickPosition:', { x, y });
    
    toast.info(`Đã chọn vị trí (${Math.round(x)}, ${Math.round(y)}). Bấm thêm component để đặt tại đây.`);
  };
              ...
              <main className="flex-1 bg-slate-950 flex flex-col items-center justify-start p-6 overflow-auto relative transition-all">
                ...
                >
                  <CanvasRenderer
                    ...
                    onCanvasClick={(x, y) => {
                      console.log('🔵 ADMIN: onCanvasClick được gọi với:', { x, y });
                      handleCanvasClick(x, y, canvasContainerRef.current);
                    }}
                  />
                </div>
              </main>
